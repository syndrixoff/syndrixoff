  ...
  document.querySelectorAll('.team-card').forEach(card => {
    ...
    if (placeholder && !img) {
      ...
      if (nameEl) {
        ...
        placeholder.innerHTML = `<span style="font-family:var(--font);font-size:2rem;font-weight:700;color:var(--copper);opacity:0.9">${initials}</span>`;
    ...
    if (img) {
      img.addEventListener('error', () => {
        ...
        if (nameEl && parent) {
          ...
          div.innerHTML = `<span style="font-family:var(--font);font-size:2rem;font-weight:700;color:var(--copper);opacity:0.9">${initials}</span>`;
...
function initFilters() {
  ...
  let emptyState = teamGrid?.parentElement?.querySelector('.team-empty');
  if (!emptyState && teamGrid) {
    emptyState = document.createElement('div');
    emptyState.className = 'team-empty';
    emptyState.setAttribute('aria-live', 'polite');
    emptyState.innerHTML = '<p>No team members match this filter.</p>';
    teamGrid.insertAdjacentElement('afterend', emptyState);
  }
